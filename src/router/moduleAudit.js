...
// 新增数据
router.post("/package/item/add1", koaBody(), async (ctx) => {
...
// 更新列表数据 /
router.post("/package/item/audit/audit", koaBody(), async (ctx) => {
  let getData = ctx.request.body;
  let readData = fs.readFileSync(
    path.join(__dirname, `../public/data/mine-component.json`),
    "utf-8"
  ...
  ctx.body = { msg: getData.status == 2 ? "审核通过" : "审核被驳回" };
});

// 删除数据
router.post("/package/item/delete1", koaBody(), async (ctx) => {
  let getData = ctx.request.body;
  let readData = fs.readFileSync(
    path.join(__dirname, `../public/data/mine-component.json`),
    "utf-8"
...
// 获取当前数据详情
router.get("/package/item/detail1", koaBody(), async (ctx) => {
...
// 获取列表数据 /
router.post("/package/item/audit/page/list", koaBody(), async (ctx) => {
